...
def run(config):
    ...
    shutil.rmtree(args.proj_dir, ignore_errors=True)
    try:
...
"""
    ...
    with patch(
        ...
        "pytorch_lightning.strategies.deepspeed.DeepSpeedStrategy._set_node_environment_variables",
        lambda self: None,
    ), patch(
        "pytorch_lightning.trainer.connectors.accelerator_connector.AcceleratorConnector._choose_and_init_cluster_environment",
        lambda self: RayEnvironment(),
    ):
