...
def run_on_every_node(remote_func_or_actor_class, *remote_args, **remote_kwargs):
    ...
    for node in ray.nodes():
        if node["Alive"] and node["Resources"].get("GPU", None):
            refs.append(
                force_on_node(node["NodeID"], remote_func_or_actor_class).remote(*remote_args, **remote_kwargs)
            )
    return ray.get(refs)
...
def download_model(base_model_name=None):
    base_model_name = base_model_name or "RWKV-4-Pile-1B5" # "RWKV-4-Pile-1B5", "RWKV-4-Pile-430M", "RWKV-4-Pile-169M"
...
def download_pile_remote(dataset_name):
    subprocess.run(
       "rm -rf /nvme/enwik8; rm -rf /nvme/data/pile/; rm -rf ~/gpt-neox", shell=True, check=True
    )
    subprocess.run(
...
def clean_cache():
    subprocess.run(
        "rm -rf  ~/.cache/torch_extensions", shell=True, check=True
    )
